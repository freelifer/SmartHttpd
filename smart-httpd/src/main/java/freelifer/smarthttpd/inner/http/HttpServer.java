...
public class HttpServer implements Runnable {
    ...
    private Logger logger = Logger.getLogger(false, HttpServer.class);
    ...
    public void run() {
        try {
            ...
            try {
                ...
                logger.e("绑定端口失败,请检查server.xml中是否设置了port属性");
                return;
            }
            logger.i("成功绑定端口" + portStr);
            //将通道设置为非阻塞模式
                ...
                while (iterator.hasNext()) {
                    ...
                    if (key.isAcceptable()) {
                        ...
                        if (socketChannel != null) {
                            logger.i("收到了来自" + ((InetSocketAddress) socketChannel.getRemoteAddress()).getHostString()
                                    + "的请求");
